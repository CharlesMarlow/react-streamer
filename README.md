...
## In order to run this project and work with it please follow the following

- json-server is used in this project to serve as a database which is easy
  and fast to set up.
  In the root folder of the project's file tree open up a terminal of your choice and run `./init.sh`.

This script will install the server's dependencies and will run it. It will then install the app's
dependencies as well and run the app, available on port 3000.

This terminal window can also serve as your visual indication to actions being performed on the
front-end, and it provides info regarding the type of request made (REST action), its corresponding
status code and the url which was handled.
